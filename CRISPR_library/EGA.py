...
class Individual:
    ...
    def combineSeq(individual,self):
        sequence_parts = [self.dict20nt[gene][index - 1] for gene, index in zip(list(self.dict20nt.keys()), individual.index)]
...
# Define the roulette selection function
...
class Ga:
    def __init__(self, config):
        ...
        self.GeneMaxIndices = config['numSpacerList']
        self.tournamentSize = config['tournamentSize']
        self.generation = 1
        
        # Gene sequence parameters
        self.dict20nt = config['spacerDist']
        self.RIBOJ = config['insulatorSeq'].replace('T', 'U')
        self.Repeat = config['spacerSeq']
        
        random.seed(42)

        # Collect the fitness value of each generation, 
        # the average fitness value of each generation group, 
        # and the best individuals of each generation
        self.uniqueMFEs = []
        self.average_MFEs = []
        self.minimumMFEs = []
        ...
        print(
            f"Generation {self.generation}/{self.termination},population size:{len(self.population)},best individual index:{self.best_parent_individual.index},best MFE:{self.best_parent_individual.fitness},average MFE:{self.average_MFEs[-1]}")
    ...
    def iter_once(self):
        # Select parent (use tournament selection)
        ...
        #  If the lowest of the parent is less than the lowest of the offspring, 
        ...
        return self.minimumMFEs
    
    
    def sort(self):
        end_time = time.time()
        runtime = round(end_time - self.start_time, 1)

        self.all_individuals_sorted = sorted(self.all_individuals, key=lambda ind: ind.fitness)
       
        result = []
        for individual in self.all_individuals_sorted:
            formatted_individual = {
                "freeEnergy": individual.fitness,  
                "geneList": []
            }
            
            formatted_insulatorPart = {
                "type": "insulator",
                "sequence": self.RIBOJ
            }
            
            formatted_spacerPart = {
                "type": "spacer",
                "sequence": self.Repeat
            }

            formatted_individual["geneList"].extend([formatted_insulatorPart, formatted_spacerPart])
            
            for i,gene_index in enumerate(individual.index):
                geneName = list(self.dict20nt.keys())[i]
                formatted_genePart = {
                "type": geneName,
                "sequence": self.dict20nt[geneName][gene_index - 1]
                }
                
                formatted_individual["geneList"].extend([formatted_genePart, formatted_spacerPart])
            
            result.append(formatted_individual)
        return result
