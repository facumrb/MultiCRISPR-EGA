...
def findRuns(lst):#用于识别序列中连续出现的错配
...
def calcCropitScore(guideSeq, otSeq):
    ...
    if len(guideSeq)==23:
        ...
        warnings.warn('Running CropIT. The length of the target sequence is less than 20. This sequence will be skipped because it may not be valid for the scoring algorithm.')
        return None
    else:
        guideSeq = guideSeq[:20]
        otSeq = otSeq[:20]
        warnings.warn('Running CropIT. The length of the target sequence is not 23. Maybe your target sequence is not in the format of 20nt guide+3nt PAM, which may cause accuracy problems.')
